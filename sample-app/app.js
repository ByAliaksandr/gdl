(function() {
  ...
  const handleFormSubmit = (event) => {
    ...
    if (stepName === 'finalize') {
      ...
      publishGdlEvent(`${getStepName(stepElement)}Complete`, getStepData(stepElement));
    }
  ...
  const getStepNumber = (stepElement) => parseInt(stepElement.getAttribute('step'));

  const getStepData = (stepElement) => {
    return [ ...stepElement.querySelectorAll('input[type="text"],input[type="hidden"],select') ].reduce((result, input) => {
      return {
        ...result,
  ...
  const showStepResult = (stepElement, isValid) => {
    const resultElement = stepElement.querySelector(isValid ? '.success' : '.error');
  ...
  const disableStep = (stepElement) => {
    stepElement.querySelectorAll('select, input').forEach((element) => element.setAttribute('disabled', ''));
  };
